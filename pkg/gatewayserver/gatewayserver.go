		...
		for {
			...
			case item := <-host.handleCh:
				...
				case *ttnpb.GatewayUplinkMessage:
					...
					switch err := host.handler.HandleUplink(ctx, *gtw.GetIds(), ids, msg); codes.Code(errors.Code(err)) {
				...
				case *ttnpb.GatewayStatus:
					if err := host.handler.HandleStatus(ctx, *gtw.GetIds(), msg); err != nil {
						registerDropStatus(ctx, gtw, msg, host.name, err)
					} else {
						registerForwardStatus(ctx, gtw, msg, host.name)
					}
				case *ttnpb.TxAcknowledgment:
					if err := host.handler.HandleTxAck(ctx, *gtw.GetIds(), msg); err != nil {
